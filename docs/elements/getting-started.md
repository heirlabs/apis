...
Zero to a validated, packed, signed (**dry-run**) Desk Element in about 30 minutes using `@morbidcorp/elements-cli` and `@morbidcorp/element-sdk`.

> **Preview:** Local tooling is supported. The production element **registry is not open** for third-party submission. Use `publish --dry-run` for pack/sign verification.

See also: [Overview](/elements/) · [CLI](/elements/cli) · [Bridge API](/elements/api)
...
Errors and warnings print with paths/file:line and codes; fix anything red — so a code-level error like `E_EVAL` here comes from stage 2, not your manifest. See the [manifest reference](/elements/manifest) for every manifest code.
...
- [Bridge API](/elements/api) — every method, with examples
- [Manifest](/elements/manifest) — the manifest, field by field
