...
All element-side code talks to the desk through the object returned by `connectElement()` from `@morbidcorp/element-sdk`. Every call is validated with zod on both sides, permission-checked by the host, rate-limited (120 calls / 10 s by default), and logged.

> Protocol: `heir-element-api@1` (closed method set).
